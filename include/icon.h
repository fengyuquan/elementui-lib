...
{
public:
    // 注意：此 enum 的順序必須與 icon.cpp 中 _iconList 的順序一致（index 對應 svg 檔名）
    enum Name {
        None = -1,
...
public:
    ...
    static bool isNone(Name icon) { return icon == None; }
    ...
    class QSvgRenderer* getRenderer(Name icon, const QString& color, QObject* parent);

private:
    FIcon();
    FIcon(const FIcon&) = delete;
    FIcon& operator=(const FIcon&) = delete;
...
private:
    // name,color,bgColor,size => icon
    QMap<std::tuple<QString, QString, QString, int>, QIcon> _iconCache;
